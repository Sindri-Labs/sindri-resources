...
// We also implement methods to select the index of the leaf for proving inclusion and for caculuating the depth of the Merkle tree.
impl InputData {
...
pub fn verify_merkle_proof_circuit(
    ...
    // Public inputs corresponding to nodes in the Merkle path are specified have to specified as HashOutTarget types.
    ...
    // The hash output of the base layer must be inputted correctly in either the left or right position for the next hashing operation.
    ...
    // Similarly, the hash output of the previous layer must be inputted in the correct position for the subsequent layers of the Merkle tree to ensure the correct hash is computed.
